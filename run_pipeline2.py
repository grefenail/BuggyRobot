"""
Pipeline 2 - scanline lane detector.

Run:
    python buggyvis1/run_pipeline2.py
    python buggyvis1/run_pipeline2.py IMG_6744.MP4
    python buggyvis1/run_pipeline2.py --headless IMG_6744.MP4
    python buggyvis1/run_pipeline2.py --live-input --headless --publish-ros
...
sys.path.insert(0, str(ROOT / "pipeline2"))
...
def play(video_path=None, print_waypoints=False, print_every=1, headless=False,
    ...
    WIN_MAIN = "Pipeline 2 - Scanline Lane Detector"
    if not headless:
    ...
    while True:
        ...
        if not headless and debug and last_steps is not None:
            mask, edges, scanlines, bird = last_steps
            imgs = [
                label(bird, "Step 0 - Bird's-eye perspective"),
                label(mask, "Step 1 - Relative white mask", is_gray=True),
                label(edges, "Step 2 - Canny edges", is_gray=True),
                label(scanlines, "Step 3 - Scanline pairs"),
            ]
